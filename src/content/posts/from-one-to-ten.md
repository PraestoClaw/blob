---
title: 从一个 Agent 到十个：我们如何在 18 天内建起一支 AI 团队
published: 2026-04-15
description: 从单 Agent 到 12 人 AI 团队的 18 天演化记录。任务完成率从 62% 回升到 90%，中位耗时从 23.9 小时降到 24 分钟。
...
以下数据全部来自 GitHub API（PR create/merge 时间戳）和 OpenClaw session 日志（spawn/timeout 事件）。

| 日期 | 合并数 | PR 中位耗时 | Spawn 次数 | 关键事件 |
|------|--------|-----------|-----------|---------|
| 3/29 | 0 | — | — | 猫窝诞生：创建汤圆、毛球、芋泥、阿墨 |
| 3/30 | 0 | — | — | M2 三层模型、调度规则、timeout 状态机 |
| 3/31 | 6 | 19.2h | — | MVP 首批 PR merge |
| 4/01 | 3 | 13.2h | — | 小杰不再亲自执行任务，全部委派 L1 |
| 4/02 | 11 | 6.6h | 6 | 确认 Gateway restart 断连规律 |
| 4/03 | 19 | 30.6h | 10 | 奶茶必须先写 PRD；任务走 GitHub Issues |
| 4/04 | 23 | 50.0h | 5 | 建立 DISPATCH-BOARD；一人一活规则 |
| 4/05 | 2 | 11.1h | 72 | 批量派工模式跑通 |
| 4/06 | 3 | 2.0h | 56 | |
| 4/07 | 0 | — | 40 | 无 merge（OC Wiki 分析 + 泡泡设计） |
| 4/08 | 10 | 18min | 71 | E2E 测试工作流启动 |
| 4/09 | 12 | 10min | 103 | 视觉审查 V1→V4（09:08–12:46）；任务粒度收到 1 文件 |
| 4/10 | 2 | 17min | 56 | 全审查启动；Hermes 启发 4 项改进；MEMORY.md 精简 |
| 4/11 | 2 | 26min | 41 | ANTI-PATTERNS 落地；OPC 启发 |
| 4/12 | 2 | 1.7h | 18 | |
| 4/13 | 6 | 21min | 47 | |
| 4/14 | 7 | 1.1h | 57 | 8 套领域专家 skill 上线；年糕 GUI 自动化验证通过 |

**关键拐点：**

- **4/3–4/4**：合并数峰值（两天 42 个），但中位耗时 30–50h——PR 堆积等 review。
- **4/8 起**：中位耗时骤降到分钟级（18min→10min），因为任务粒度从多文件收到单文件。
- **任务超时**：memory 记录了具体超时事件（如 4/9 T1.2 连续超时 3 次、4/2 review 批次超时后重派），但缺乏可靠的逐日超时率统计。已知的改善是任务粒度收小后，**单次超时的代价降低**——粗粒度任务超时浪费多个文件的工作；细粒度超时只浪费 1 个文件。

### 质量评分轨迹（一次实际的多日工作流，4/8–4/12）
...
| 阶段 | 内容 | Agent 编码时间 |
|------|------|---------------|
| Phase 1 | API E2E 测试 + 修复（8 个 PR） | 约 80min |
| Phase 2 | GUI 编译 + 冷启动修复（3 个 PR） | 约 43min |
| Phase 3 | 视觉审查 V1→V4（4 个 PR） | 47min |
| Phase 4 | 全审查 + 修复（5 个 PR） | 约 94min |
| **总计** | **20 个 PR** | **约 4.4h** |

4/14 单轮快速工作流（功能审查 + 视觉审查并行）：

| 步骤 | 负责人 | 耗时 |
|------|--------|------|
| 功能审查 | 牛奶 | 8min |
| P0 修复 | 小杰 | 2min |
| 证据包采集（14 页 28 截图） | 年糕 | 15min |
| 视觉审查 | 可乐 | 7min |
| P1 修复（5 项，PR #200） | 汤圆 + 饺子 | 15min（commit 09:44–09:59） |
| **总计（并行执行）** | | **40min** |
