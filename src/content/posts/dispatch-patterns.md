...
**翻车案例**：汤圆完成了一个任务，但小杰在处理其他消息，没有及时检查队列。汤圆空闲了 30 分钟，而队列里有 3 个任务在等。
...
**翻车案例**：`openclaw gateway restart` 后，3 个 subagent 的状态仍显示 running，但实际已经断连，不再推进。白等了 1 小时才发现。
...
**翻车案例**：DISPATCH-BOARD 说汤圆在忙，但实际上汤圆的 subagent 已经完成了。结果新任务排队等了 20 分钟，而汤圆一直空闲。
...
**翻车案例**：一个任务改 4 个 service + main.py，当成一个单元派出去。连续超时 3 次才意识到需要拆。拆成 5 个单文件任务后，每个都在 15 分钟内完成。
